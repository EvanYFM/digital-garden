...
| `notes.html` | 思考：文章索引（无序号 · 时间流 · 排序切换 · 每 10 篇分页；含用户文章）+ 随记（稳定锚点 `#f时间戳`；被移植后显示「已长成」回链，登记于 `FRAG_GROWN`） |
...
| `review.html` | 复盘：循环示意 + 决策与交易（链接至期货工作站）+ 每日此刻索引（私密区块，仅本人设备可见）+ 复盘记录（无序号 · 时间流 · 排序 · 分页） |
...
随记（notes.html 种下）→ 写作台（write.html 写成文章，修订留痕）→ 发布（公开仓库 `user-articles.json`，全站可见）→ 常青。用户文章 id 形如 `u<毫秒时间戳>`，草稿私有（仅私库与本机），发布内容才进公开仓库。文章页对本人设备显示「编辑/删除/写作台」操作条；删除采用墓碑（tombstone `{deleted:true}`）随合并传播，渲染层一律跳过；删除已发布文章会同步推新 user-articles.json 撤下公开版。
